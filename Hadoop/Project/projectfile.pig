-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/radhika/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/radhika/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/radhika/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/radhika/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/radhika/outputs' USING PigStorage('\t');
